...
function TrendIcon({ trend }: { trend: string }) {
  if (trend === "up" || trend === "↑" || trend === "上升") return <TrendingUp size={18} className="text-[var(--danger)]" />
  if (trend === "down" || trend === "↓" || trend === "下降") return <TrendingDown size={18} className="text-[var(--success)]" />
...
export function RiskResponseCenter() {
  ...
  const [factors, setFactors] = useState("监管合规, AI算力, 国际化经营")
  ...
  const fetchRisk = useCallback(async () => {
    ...
    const response = await evaluateRisk({
      ...
      confidence,
      factors: factors.split(",").map((item) => item.trim()).filter(Boolean),
    })
    setData(response)
    setLoading(false)
  }, [confidence, factors, portfolioValue])
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--text-muted)]">正在加载评估模型...</p>
        </div>
        ...
        <Card variant="elevated" glow padding="lg" className="flex flex-col items-center justify-center text-center">
          ...
          <Skeleton className="mt-3 h-4 w-24 rounded-lg" />
        </Card>
  ...
  const recommendedAction =
    data.risk_score >= 75
      ? "建议立刻切换到稳健方案，冻结新增高风险敞口并加强监管事项应对。"
      : data.risk_score >= 50
        ? "系统推荐平衡响应方案，优先覆盖高贡献风险因子并保留核心增长弹性。"
        : "当前风险处于低位，可维持常规监控并等待新信号触发。"
  ...
  const distOption = {
    ...
    xAxis: {
      type: "value",
      name: "风险评分",
      nameTextStyle: { color: "#726d8c", fontSize: 10 },
      axisLine: { lineStyle: { color: "rgba(139,132,190,0.12)" } },
      axisLabel: { color: "#726d8c", fontSize: 10 },
    },
    yAxis: {
      type: "value",
      name: "密度",
      nameTextStyle: { color: "#726d8c", fontSize: 10 },
      splitLine: { lineStyle: { color: "rgba(139,132,190,0.06)" } },
      axisLabel: { color: "#726d8c", fontSize: 10 },
    },
    series: [
      {
        type: "line",
        data: data.probability_distribution.map((item) => [item.x, item.y]),
        smooth: true,
        symbol: "none",
        areaStyle: {
          color: {
            ...
            y2: 1,
            colorStops: [
              { offset: 0, color: "rgba(165,153,240,0.25)" },
              { offset: 1, color: "rgba(165,153,240,0.02)" },
            ],
          },
        },
        ...
        markLine: {
          ...
          label: { formatter: `VaR: ${data.risk_score}`, color: "#f87171", fontSize: 10 },
          data: [{ xAxis: data.risk_score }],
        },
      },
    ],
    tooltip: {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-[-0.02em] text-white">智能风险评估中心</h1>
          <p className="mt-1 text-xs text-[var(--text-muted)]">蒙特卡洛模拟 + VaR + 因子贡献 · 实时风险评估</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card variant="soft" padding="sm">
          ...
          <div className="mt-3 rounded-lg border border-white/[0.05] bg-white/[0.025] px-3 py-2 text-[10px] text-[var(--text-muted)]">
            风险爆发窗口：
            <span className="font-mono text-sky-300"> {data.time_sensitivity_hours.toFixed(1)}h</span>
          </div>
        </Card>
      ...
      <Card variant="elevated" glow padding="lg" className="border-indigo-300/14 bg-[linear-gradient(135deg,rgba(32,26,56,0.92),rgba(15,23,42,0.88))]">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] text-[var(--text-muted)]">把单一分数拆成区间、趋势和主导因子，避免只看一个裸数字。</p>
          </div>
          <div className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)]">
            <TrendIcon trend={data.trend} />
            <span>
              趋势：
              {data.trend === "up" || data.trend === "↑" || data.trend === "上升"
                ? "上升"
                : data.trend === "down" || data.trend === "↓" || data.trend === "下降"
                  ? "下降"
                  : "平稳"}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <Card variant="soft" padding="sm">
          <div className="mb-1 text-[11px] text-[var(--text-muted)]">概率分布曲线</div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <Card variant="soft" padding="sm">
          <h3 className="mb-3 text-xs font-semibold tracking-[-0.01em] text-white">VaR 风险值</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead>
                <tr className="border-b border-[rgba(139,132,190,0.08)] text-[var(--text-muted)]">
                  <th className="py-2 text-left font-medium">方法</th>
                  <th className="py-2 text-right font-medium">置信度</th>
                  <th className="py-2 text-right font-medium">VaR (%)</th>
                </tr>
              </thead>
              <tbody>
                {data.var_table.map((item, index) => (
                  <tr key={index} className="border-b border-[rgba(139,132,190,0.04)] hover:bg-white/[0.02]">
                    <td className="py-2 text-[var(--text-secondary)]">{item.method}</td>
                    <td className="py-2 text-right tabular-nums text-[var(--text)]">{item.confidence}</td>
                    <td className={`py-2 text-right font-mono tabular-nums ${item.value > 3.5 ? "text-[var(--danger)]" : item.value > 2.5 ? "text-[var(--warning)]" : "text-[var(--success)]"}`}>
                      {item.value}%
                    </td>
                  </tr>
                ))}
        ...
        <Card variant="soft" padding="sm">
          <h3 className="mb-3 text-xs font-semibold tracking-[-0.01em] text-white">因子贡献分析</h3>
          <div className="space-y-3">
            {data.factor_contributions.map((item, index) => (
              <div key={index}>
                <div className="mb-1 flex items-center justify-between">
                  <span className="text-[11px] text-[var(--text-secondary)]">{item.factor}</span>
                  <span className="font-mono text-[11px] text-white tabular-nums">{item.contribution.toFixed(1)}%</span>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.04]">
                  <div className="h-full rounded-full bg-[linear-gradient(90deg,#8b7cf0,#a599f0)] transition-all duration-700" style={{ width: `${item.contribution}%` }} />
                </div>
                <p className="mt-0.5 text-[10px] text-[var(--text-muted)]">{item.description}</p>
              </div>
            ))}
          </div>
        </Card>
